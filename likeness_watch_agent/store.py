...
def upsert_user(uid: str, email: str, name: str) -> None:
    _db.collection("users").document(uid).set({"email": email, "name": name}, merge=True)


def add_to_watchlist(uid: str, talent_name: str, known_endorsements: list[str]) -> str:
    doc_ref = _db.collection("users").document(uid).collection("watchlist").document()
    doc_ref.set({
...
def remove_from_watchlist(uid: str, doc_id: str) -> None:
    _db.collection("users").document(uid).collection("watchlist").document(doc_id).delete()


def list_watchlist(uid: str) -> list[dict]:
    docs = _db.collection("users").document(uid).collection("watchlist").stream()
...
def iter_all_watchlist_entries():
    ...
    for doc in _db.collection_group("watchlist").stream():
...
def update_watchlist_state(uid: str, doc_id: str, risk_flag: str, finding_urls: list[str]) -> None:
    _db.collection("users").document(uid).collection("watchlist").document(doc_id).update({
